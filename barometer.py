...
def get_amedas_map():
    ...
    for minutes_back in range(
        ...
        timestamp = target.strftime(
            "%Y%m%d%H%M%S"
        )

        url = JMA_MAP_URL.format(
            timestamp
        )
...
def get_nearest_observation():
    ...
    for station in stations:
        ...
        if station_id not in amedas:
            continue

        data = amedas[station_id]

        observation = station.copy()

        observation["temp"] = safe_float(
            data.get("temp")
        )

        observation["humidity"] = safe_float(
            data.get("humidity")
        )

        observation["wind"] = safe_float(
            data.get("wind")
        )

        observation["wind_direction"] = data.get(
            "windDirection"
        )
...
def get_met_forecast():
    ...
    for item in timeseries:
        ...
        try:
            ...
            next_1h = data_block.get(
                "next_1_hours",
                {}
            )

            next_6h = data_block.get(
                "next_6_hours",
                {}
            )
...
def calculate_pressure_changes(
    forecasts
):
...
{{ "%.1f"|format(item.pressure_change) }}
...
def index():
    ...
    all_forecasts = get_met_forecast()
