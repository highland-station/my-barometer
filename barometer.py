...
# 現地：奈良本1489-23付近（エンジェルフォレスト伊豆熱川など）
...
def index():
    ...
    highland_fallback = (
        highland_df is None or highland_df.empty
    )

    coast_fallback = (
        coast_df is None or coast_df.empty
    )
    ...
    df["CoastTemp"] = df["CoastTemp"].fillna(
        df["Temp"]
    )
    ...
    if data_error:
        ...
        message = (
            f"<b>🔴 気圧警戒："
            f"{current_forecast_press:.1f} hPa</b>"
            "<br>"
            "気圧変化に敏感な方は、"
            "無理をせずゆっくり過ごしましょう。"
        )
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="note">
                ...
                ※ 現地は標高約500mとしてOpen-Meteoから取得しています。<br>

                ※ 気圧は「現地気圧」と「海面気圧（予報基準）」を表示しています。<br>

                ※ 麓の気温は伊豆熱川駅付近のデータです。<br>
